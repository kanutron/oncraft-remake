...
<template>
  <div :data-mode="mode" :data-error="isError" :class="mode === 'badge' ? 'inline-flex' : 'block my-2'">
    <template v-if="mode === 'badge'">
      <UBadge :color="isError ? 'error' : 'success'" variant="subtle" class="cursor-pointer" @click="cycleMode()">
    ...
    <template v-else-if="mode === 'compact'">
      <div
        class="flex items-center gap-2 text-xs border-t border-neutral-200 dark:border-neutral-800 pt-2 cursor-pointer"
        @click="cycleMode()"
      >
